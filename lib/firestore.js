...
class Firestore {
	...
	all(model, filter, callback) {
		...
		if (filter && filter.where) {
			if (filter.where.id) {
				...
				this.buildQuery(filter, this.db.collection(model), query => {
	...
	buildQuery(filter, query, callback) {
		...
		var response = keys.forEach(value => {
			if (value === 'where') {
				var condition = filter[value];
				var properties = Object.keys(condition);
				var operator = '==';

				if (typeof(filter[value][properties]) == 'object') {
					var restOperator = Object.keys(condition[properties]);
					operator = this.decodeOperator(restOperator);
					response = query.where(properties[0], operator, condition[properties][restOperator]);
				} else {
					response = query.where(properties[0], operator, condition[properties]);
				}
				callback(response);
			} else {
				console.log('raro');

...
class Firestore {
	...
	create(model, data, callback) {
		this.db.collection(model).add(data).then(function(ref) {
	...
	buildWhereQuery(filter, query, callback) {
		...
		properties.forEach( key => {
			...
			var restOperator = Object.keys(filter[key]);
			operator = this.decodeOperator(restOperator);
			operand = filter[key][restOperator];
		} else {
			operand = filter[key];
		}
		if(!count ){
			compound.push(query.where(key, operator, operand))

		}else{
			compound.push(compound[0].where(key, operator, operand))
		...
		callback(compound[compound.length-1]);
	}

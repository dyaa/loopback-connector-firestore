	...
	if (this.serviceAccountKey) {
		...
		admin.initializeApp({
			credential: admin.credential.cert({
				...
				privateKey: dataSourceProps.privateKey,
			}),
			databaseURL: 'https://' + dataSourceProps.databaseName + '.firebaseio.com',
		});
	}
 ...
 */
Firestore.prototype.destroyAll = function(model, where, callback) {
	var self = this;
...
Firestore.prototype.deleteQueryBatch = function(db, query, batchSize, resolve, reject) {
	query.get()
		.then((snapshot) => {
...
Firestore.prototype.deleteCollection = function(db, collectionPath, batchSize) {
